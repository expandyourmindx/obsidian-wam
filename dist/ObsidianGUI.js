...
function Knob({ value = .5, onChange, onReset, size = "md", label, fmt }) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			(active || hover) && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				style: {
					position: "absolute",
					top: -26,
					left: "50%",
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
				...
				children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						d: ap(cx, cy, cr, -90, 90),
						fill: "none",
						stroke: "rgba(255,225,190,0.07)",
						strokeWidth: "1.5"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						d: ap(cx, cy, cr, 90, 270),
						fill: "none",
					...
					ticks,
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						d: ap(cx, cy, ar, -135, 135),
						fill: "none",
						stroke: T.borderSubtle,
						strokeWidth: "2"
					}),
					value > .005 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						d: ap(cx, cy, ar, -135, -135 + value * 270),
						fill: "none",
						stroke: arcColor,
						strokeWidth: "2"
					}),
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						d: ap(cx, cy, fr, 90, 270),
						fill: "none",
						stroke: "rgba(255,225,190,0.04)",
						strokeWidth: "1"
					}),
	...
	const pts = Array.from({ length: 30 }, (_, i) => {
		...
		switch (wave) {
			...
			case "saw":
				...
				break;
			case "square":
				y = Math.sin(φ) >= 0 ? .82 : -.82;
				break;
			case "triangle":
				y = 2 / Math.PI * Math.asin(Math.sin(φ));
				break;
			default: y = 0;
		}
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("line", {
					...
					x2: W,
					y2: H / 2,
					stroke: T.borderGhost,
					strokeWidth: "0.5"
				}),
				active && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
					d: pts,
					fill: "none",
					stroke: "rgba(196,43,43,0.2)",
					strokeWidth: "3"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
					d: pts,
					...
					strokeWidth: active ? 1.2 : .8
				})
			]
		})
	});
}
function FilterDisplay({ W = 640, H = 80, cutoff = .5, res = .5 }) {
	const cvs = (0, import_react.useRef)(null);
...
function ADSRDisplay({ attack = .5, decay = .5, sustain = .5, release = .5, W = 130, H = 40 }) {
...
function ObsidianPanel({ wam }) {
	...
	(0, import_react.useEffect)(() => {
		...
		function draw(ts) {
			...
			for (let x = 0; x < W; x++) {
				...
				const peak3 = .2 * Math.exp(-Math.pow(normX - (.6 + .12 * Math.sin(t * .9)), 2) * 50);
				const noise = .05 * Math.sin(x * .5 + t * 20) * Math.sin(x * .05 - t * 10);
				const jitter = .02 * Math.random();
				const val = peak1 + peak2 + peak3 + noise + jitter + .05;
	...
	const updateParam = (key, rawValue) => {
		...
		else if (key === "osc1Coarse" || key === "osc2Coarse" || key === "osc3Coarse") paramValue = (rawValue - .5) * 48;
		else if (key === "osc1Fine" || key === "osc2Fine" || key === "osc3Fine") paramValue = (rawValue - .5) * 200;
		else if (key === "osc1Pan" || key === "osc2Pan" || key === "osc3Pan") paramValue = (rawValue - .5) * 2;
		else if (key === "lfoRate") paramValue = .1 + rawValue * 19.9;
		else if (key === "pitchEnvAmount") paramValue = (rawValue - .5) * 48;
		else if (key === "unisonVoices") paramValue = Math.max(1, Math.round(rawValue * 8));
		else if (key === "unisonDetune") paramValue = rawValue * 100;
		else if (key === "stereoWidth") paramValue = rawValue * 2;
		else if (key === "portamentoTime") paramValue = rawValue * 2;
		else if (key === "filterEnvAmount") paramValue = (rawValue - .5) * 2;
		if (wam) wam.setParam(key, paramValue);
		setParams((prev) => prev ? {
			...prev,
			[key]: paramValue
		} : null);
	...
	const getKnobVal = (key) => {
		if (!params) return DEFAULT_PARAMS[key] !== void 0 ? convertToKnob(key, DEFAULT_PARAMS[key]) : .5;
		const val = params[key];
		if (val === void 0) return .5;
		return convertToKnob(key, val);
	};
	const convertToKnob = (key, val) => {
		if (key === "filterResonance") return val / 3.8;
		else if (key === "osc1Coarse" || key === "osc2Coarse" || key === "osc3Coarse") return val / 48 + .5;
		else if (key === "osc1Fine" || key === "osc2Fine" || key === "osc3Fine") return val / 200 + .5;
		else if (key === "osc1Pan" || key === "osc2Pan" || key === "osc3Pan") return val / 2 + .5;
		else if (key === "lfoRate") return (val - .1) / 19.9;
		else if (key === "pitchEnvAmount") return val / 48 + .5;
		else if (key === "unisonVoices") return val / 8;
		else if (key === "unisonDetune") return val / 100;
		else if (key === "stereoWidth") return val / 2;
		else if (key === "portamentoTime") return val / 2;
		else if (key === "filterEnvAmount") return val / 2 + .5;
		return val;
	};
